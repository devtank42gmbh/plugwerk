...
describe('uploadStore', () => {
  ...
  describe('addFiles', () => {
    ...
    it('filters out non-.jar/.zip files', () => {
      const files = [createFile('plugin.jar'), createFile('readme.txt'), createFile('image.png')]
      useUploadStore.getState().addFiles(files)
  ...
  describe('updateEntry', () => {
    it('updates status and progress of a specific entry', () => {
      ...
      useUploadStore.getState().updateEntry(id, { status: 'uploading', progress: 42 })
    ...
    it('updates error message on failure', () => {
      ...
      useUploadStore.getState().updateEntry(id, { status: 'failed', errorMessage: 'Server error' })
    ...
    it('does not modify other entries', () => {
      ...
      useUploadStore.getState().updateEntry(first.id, { status: 'success', progress: 100 })
  ...
  describe('clearCompleted', () => {
    it('removes success and failed entries', () => {
      useUploadStore.getState().addFiles([createFile('a.jar'), createFile('b.jar'), createFile('c.jar')])
      const [a, b, c] = useUploadStore.getState().entries

      useUploadStore.getState().updateEntry(a.id, { status: 'success' })
      useUploadStore.getState().updateEntry(b.id, { status: 'failed', errorMessage: 'err' })
      // c stays pending
